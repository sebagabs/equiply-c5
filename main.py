...
def distance(coordinates: list[dict[str, float]]) -> float:
    ...
    for i in range(len(coordinates) - 1):
        p1 = (coordinates[i].get('x', 0), coordinates[i].get('y', 0), coordinates[i].get('z', 0))
        p2 = (coordinates[i+1].get('x', 0), coordinates[i+1].get('y', 0), coordinates[i+1].get('z', 0))
        result += math.dist(p1, p2)
...
def velocity(coordinates: list[dict[str, float]]) -> dict[str, float]:
    ...
    return {
        'x': round((coordinates[len(coordinates)-1].get('x', 0) - coordinates[0].get('x', 0)) / time_elapsed, 2) if time_elapsed > 0 else 0,
        'y': round((coordinates[len(coordinates)-1].get('y', 0) - coordinates[0].get('y', 0)) / time_elapsed, 2) if time_elapsed > 0 else 0,
        'z': round((coordinates[len(coordinates)-1].get('z', 0) - coordinates[0].get('z', 0)) / time_elapsed, 2) if time_elapsed > 0 else 0,
    }
